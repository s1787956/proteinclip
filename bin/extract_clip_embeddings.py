...
def build_parser():
    ...
    parser.add_argument(
        "--text_path",
        type=str,
        default="../data/uniprot_text_embed.text-embedding-3-large.hdf5",
        help="Path text embeddings to extract embeddings from with pretrained text projection network. Should be a folder contining metrics.csv and checkpoints/ folder.",
    )
    parser.add_argument(
        "--feat_path",
        type=str,
...
def main():
    ...
    for f in tqdm(files):
        assert os.path.exists(os.path.join(args.feat_path,f"{f}.h5")), f'{os.path.join(args.feat_path,f)}.h5 Not found!'
        prot_emb = np.array(h5py.File(os.path.join(args.feat_path,f"{f}.h5"),'r')["features_mean"][:])
